...
class AggExpr : public Expr {
 public:
  AggExpr(const SQLTypeInfo& ti,
      ...
      : Expr(ti, true), aggtype(a), is_distinct(d), error_rate(e) {
    if (g) {
      args.push_back(g);
    }
  }
  ...
  Expr* get_arg() const { return !args.empty() ? args.front().get() : nullptr; }
  std::shared_ptr<Analyzer::Expr> get_own_arg() const { return !args.empty() ? args.front() : nullptr; }
  const std::vector<std::shared_ptr<Analyzer::Expr>>& get_own_args() const { return args; }
  Expr* get_arg_at(size_t i) const { return args[i].get(); }
  std::shared_ptr<Analyzer::Expr> get_own_arg_at(size_t i) const { return args[i]; }
 ...
 private:
  SQLAgg aggtype;                                     // aggregate type: kAVG, kMIN, kMAX, kSUM, kCOUNT, kFAGMS_SKETCH
  std::vector<std::shared_ptr<Analyzer::Expr>> args;  // argument(s) to aggregate
