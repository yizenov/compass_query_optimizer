...
std::shared_ptr<Analyzer::Expr> AggExpr::deep_copy() const {
  std::vector<std::shared_ptr<Analyzer::Expr>> args_new(args.size());
  for (auto arg : args) {
    args_new.emplace_back(arg->deep_copy());
  }
  return makeExpr<AggExpr>(type_info, aggtype, args, is_distinct, error_rate);
}
...
std::shared_ptr<Analyzer::Expr> AggExpr::rewrite_with_child_targetlist(
  ...
  std::vector<std::shared_ptr<Analyzer::Expr>> args_new(args.size());
  for (auto arg : args) {
    args_new.emplace_back(arg->rewrite_with_child_targetlist(tlist));
  }
  return makeExpr<AggExpr>(type_info, aggtype, args_new, is_distinct, error_rate);
}
...
bool AggExpr::operator==(const Expr& rhs) const {
  ...
  if (aggtype != rhs_ae.get_aggtype() || is_distinct != rhs_ae.get_is_distinct())
    return false;
  if (args.size() == rhs_ae.get_num_args()) {
    for (size_t i = 0; i < args.size(); i++) {
      if (args[i].get() != rhs_ae.get_arg_at(i)) {
        return false;
      }
    }
    return true;
