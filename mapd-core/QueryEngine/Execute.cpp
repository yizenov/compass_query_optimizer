...
int32_t Executor::executePlanWithoutGroupBy(const RelAlgExecutionUnit& ra_exe_unit,
  ...
  if (device_type == ExecutorDeviceType::CPU) {
    OOM_TRACE_PUSH();
    bool iskFAGMS_SKETCH = false;
    for (const auto target_expr : target_exprs) {
      const auto agg_info = target_info(target_expr);
      CHECK(agg_info.is_agg);
      if (agg_info.agg_kind == kFAGMS_SKETCH) {
        iskFAGMS_SKETCH = true;
        auto sketch_elements = getSketch().second->get_sketch_elements();
        auto sketch_size = getSketch().second->get_rows_no() * getSketch().second->get_buckets_no();
        for (unsigned int i = 0; i < sketch_size; i++)
          query_exe_context->init_agg_vals_.push_back(sketch_elements[i]);
        break;
      }
    }
    std::cout << "cpu setup is being processed" << std::endl;

    auto out_vec_temp = query_exe_context->launchCpuCode(ra_exe_unit,
    ...
    if (iskFAGMS_SKETCH) {
      double* sketch_elements = getSketch().second->get_sketch_elements();
      auto sketch_size = getSketch().second->get_rows_no() * getSketch().second->get_buckets_no();

      // update original sketch values after each thread with 'Addition operation'
      for (unsigned int i = 0; i < sketch_size; i++)
        sketch_elements[i] += query_exe_context->init_agg_vals_[i + 1];
      query_exe_context->init_agg_vals_ = {query_exe_context->init_agg_vals_[0]};
      out_vec = {out_vec_temp[0]};
    } else {
      out_vec = out_vec_temp;
    }
    ...
    try {
      ...
      bool iskFAGMS_SKETCH = false;
      std::vector<int64_t> initial_agg_vals = query_exe_context->init_agg_vals_;
      for (const auto target_expr : target_exprs) {
        const auto agg_info = target_info(target_expr);
        CHECK(agg_info.is_agg);
        if (agg_info.agg_kind == kFAGMS_SKETCH) {
          ...
          uint64_t data_alloc_size = gridSize() * (blockSize()
                  + getSketch().second->get_rows_no() * getSketch().second->get_buckets_no() * getSketch().second->get_keys_no());
          std::vector<int64_t> vec(data_alloc_size, 0);
          query_exe_context->init_agg_vals_ = vec;
          int64_t sketch_gpu_init_stop = timer_stop(sketch_gpu_init_begin);
          std::cout << "time taken for initial sketch values setup in gpu: " << sketch_gpu_init_stop << std::endl;
          break;
        }
      }

      auto sketch_gpu_init_begin = timer_start();
      auto out_vec_temp = query_exe_context->launchGpuCode(ra_exe_unit,
                                                           compilation_result.native_functions,
                                                           hoist_literals,
                                                           hoist_buf,
                                                           col_buffers,
        ...
        int64_t sketch_gpu_init_stop = timer_stop(sketch_gpu_init_begin);
        std::cout << "time taken for actual sketch update in gpu: " << sketch_gpu_init_stop << std::endl;

      if (iskFAGMS_SKETCH) {
        double** separate_sketch_elements = getSketch().second->get_separate_sketch_elements();
        auto sketch_size = getSketch().second->get_rows_no() * getSketch().second->get_buckets_no();
        unsigned int column_number = getSketch().second->get_keys_no();

        // update original sketch values after each thread with 'Addition operation'
        int64_t* updated_sketch = out_vec_temp[1];
        for (unsigned int i = 0; i < column_number; i++) {
          uint64_t col_sketch_size = i * sketch_size;
          double* column_sketch = new double[sketch_size];
          for (unsigned int j = 0; j < sketch_size; j++) {
            column_sketch[j] = updated_sketch[col_sketch_size + j];
          }
          separate_sketch_elements[i] = column_sketch;
        }

        query_exe_context->init_agg_vals_ = initial_agg_vals;
        out_vec = {out_vec_temp[0]};
      } else {
        out_vec = out_vec_temp;
      }
