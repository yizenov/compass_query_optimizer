...
class RelAlgAbstractInterpreter {
 public:
  ...
  std::shared_ptr<const RelAlgNode> run() {
    ...
    std::cout << "dispatchNodes" << std::endl;
    //std::cout << tree_string(nodes_.back().get(), 0) << std::endl;

    bind_inputs(nodes_);
    std::cout << "bind_inputs" << std::endl;
    //std::cout << tree_string(nodes_.back().get(), 0) << std::endl;

    mark_nops(nodes_);
    std::cout << "mark_nops" << std::endl;
    //std::cout << tree_string(nodes_.back().get(), 0) << std::endl;

    simplify_sort(nodes_);
    std::cout << "simplify_sort" << std::endl;
    //std::cout << tree_string(nodes_.back().get(), 0) << std::endl;

    sink_projected_boolean_expr_to_join(nodes_);
    std::cout << "sink_projected_boolean_expr_to_join" << std::endl;
    //std::cout << tree_string(nodes_.back().get(), 0) << std::endl;

    eliminate_identical_copy(nodes_);
    std::cout << "eliminate_identical_copy" << std::endl;
    //std::cout << tree_string(nodes_.back().get(), 0) << std::endl;

    fold_filters(nodes_);
    std::cout << "fold_filters" << std::endl;
    //std::cout << tree_string(nodes_.back().get(), 0) << std::endl;
    ...
    if (filtered_left_deep_joins.empty()) {
      std::cout << "filtered_left_deep_joins" << std::endl;
      //std::cout << tree_string(filtered_left_deep_joins.back(), 0) << std::endl;

      hoist_filter_cond_to_cross_join(nodes_);
      std::cout << "hoist_filter_cond_to_cross_join" << std::endl;
      //std::cout << tree_string(nodes_.back().get(), 0) << std::endl;
    }

    eliminate_dead_columns(nodes_);
    std::cout << "eliminate_dead_columns" << std::endl;
    //std::cout << tree_string(nodes_.back().get(), 0) << std::endl;

    if(ra_executor_->fpd_enabled()) {
      push_down_filter_predicates_sketch(nodes_, ra_executor_);
      std::cout << "push_down_filter_predicates" << std::endl;
      //std::cout << tree_string(nodes_.back().get(), 0) << std::endl;
    }

    coalesce_nodes(nodes_, left_deep_joins);
    CHECK(nodes_.back().unique());
    std::cout << "coalesce_nodes" << std::endl;
    //std::cout << tree_string(nodes_.back().get(), 0) << std::endl;

    create_left_deep_join(nodes_);

    std::cout << "Final: create_left_deep_join" << std::endl;
    //std::cout << tree_string(nodes_.back().get(), 0) << std::endl;
