...
class RexAgg : public Rex {
 public:
  RexAgg(const SQLAgg agg, const bool distinct, const SQLTypeInfo& type,
	const std::vector<size_t>& operands) : agg_(agg), distinct_(distinct),
		type_(type), operands_(operands) {}
...
class RelProject : public RelAlgNode {
 public:
  ...
  void setExpressions(std::vector<std::unique_ptr<const RexScalar>>& exprs) {
...
class RelAggregate : public RelAlgNode {
 public:
  ...
  RelAggregate(const size_t groupby_count,
               ...
               const std::vector<std::string>& fields,
               std::shared_ptr<const RelAlgNode> input)
      : groupby_count_(groupby_count), agg_exprs_(std::move(agg_exprs)),
		fields_(fields) {
    inputs_.push_back(input);
  }
 ...
 public:
  ...
  JoinType getJoinType() const {
    return sequence_.empty() ||
    	sequence_.back() == nullptr ? JoinType::INVALID : sequence_.back()->getJoinType();
  }

  std::string toString() const override {
    std::string result = "RelMultiJoin<" +
    	std::to_string(reinterpret_cast<uint64_t>(this)) + ">(";
  ...
  const std::vector<std::unique_ptr<const RexScalar>>& getConditions() const {
	  return conditions_;
  }
...
class RelFilter : public RelAlgNode {
 public:
  RelFilter(std::unique_ptr<const RexScalar>& filter,
	std::shared_ptr<const RelAlgNode> input) : filter_(std::move(filter)) {
    CHECK(filter_);
    inputs_.push_back(input);
  }
...
class SortField {
 public:
  SortField(const size_t field, const SortDirection sort_dir,
