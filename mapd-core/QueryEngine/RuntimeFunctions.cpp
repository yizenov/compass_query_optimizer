...
*/
extern "C" ALWAYS_INLINE uint32_t hash31(uint64_t a, uint64_t b, uint64_t x) {
  uint64_t result;
  uint32_t lresult;
...
*/
extern "C" ALWAYS_INLINE uint32_t CW2B(uint32_t a, uint32_t b, uint64_t x, uint32_t M) {
...
*/
extern "C" ALWAYS_INLINE uint32_t seq_xor(uint32_t x) {
  x ^= (x >> 16);
  x ^= (x >> 8);
  x ^= (x >> 4);
  x ^= (x >> 2);
...
extern "C" ALWAYS_INLINE int32_t EH3(uint32_t i0, uint32_t i1, uint32_t j) {
...
extern "C" NEVER_INLINE void agg_count_and_update_sketch_gpu(int64_t* counter,
                                                                 int64_t* sketch_data,
...
extern "C" ALWAYS_INLINE int64_t* agg_count_and_update_sketch(int64_t* counter,
                                                              int64_t* sketch_data,  // starts from index 1
                                                              const uint8_t num_cols_to_update,
                                                              const int64_t* keys,  // PK-FK values
                                                              const uint32_t* col_indices,
                                                              const uint32_t* xi_bucket,
                                                              const uint32_t* xi_pm1,
                                                              const uint16_t num_buckets,
  ...
  for (unsigned i = 0; i < num_rows; i++) {
    for (unsigned j = 0; j < num_cols_to_update; j++) {
      int32_t b = CW2B(xi_bucket[2 * (num_cols_to_update * i + j)],
                       xi_bucket[2 * (num_cols_to_update * i + j) + 1],
                       keys[j],
                       num_buckets);
      sketch_data[1 + i * num_buckets + b] +=
          EH3(xi_pm1[2 * (num_cols_to_update * i + j)], xi_pm1[2 * (num_cols_to_update * i + j) + 1], keys[j]) * 1.0;
    }
  }
  counter[0]++;
  return counter;
