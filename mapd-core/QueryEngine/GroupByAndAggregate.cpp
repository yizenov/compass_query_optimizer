...
std::vector<int64_t*> QueryExecutionContext::launchGpuCode(const RelAlgExecutionUnit& ra_exe_unit,
  ...
  if (is_group_by) {
    ...
    *error_code = aggregate_error_codes(error_codes);
    ...
    bool is_fpd_sketch = false;
    for (const auto target_expr : ra_exe_unit.target_exprs) {
      const auto agg_info = target_info(target_expr);
      if (agg_info.agg_kind == kFAGMS_SKETCH) {
        is_fpd_sketch = true;
        break;
      }
    }
    ...
    if (ra_exe_unit.estimator || is_fpd_sketch)
      agg_col_count = size_t(1);
    else
      agg_col_count = init_agg_vals.size();
    ...
    *error_code = aggregate_error_codes(error_codes);
    ...
    if (is_fpd_sketch) {
      int64_t* counts = new int64_t[grid_size_x * block_size_x];

      size_t sketch_size = init_agg_vals.size() / grid_size_x - block_size_x;
      //size_t sketch_size = init_agg_vals.size() / grid_size_x / block_size_x;
      int64_t* updated_sketch = new int64_t[sketch_size];
      //for (size_t i = 0; i < sketch_size - 1; i++)
      ...
      for (size_t i = 0; i < agg_col_count; ++i) {
        int64_t* host_out_vec = new int64_t[init_agg_vals.size()];
        copy_from_gpu(
            data_mgr, host_out_vec, kernel_params[INIT_AGG_VALS], sizeof(int64_t) * init_agg_vals.size(), device_id);

        size_t counter_idx = 0;
        size_t counter_sketch_size = block_size_x + sketch_size;

        for (size_t block_idx = 0; block_idx < grid_size_x; block_idx++) {
            // sketch per block logic
            size_t block_set = counter_sketch_size * block_idx;
            // getting the counters from each thread
            for (size_t thread_idx = 0; thread_idx < block_size_x; thread_idx++) {
                counts[counter_idx] = host_out_vec[block_set + thread_idx];
                counter_idx += 1;
            }
            // reduction of sketches from each block
            size_t first_sketch_data_idx = block_set + block_size_x;
            for (size_t sketch_idx = 0; sketch_idx < sketch_size; sketch_idx++) {
              updated_sketch[sketch_idx] += host_out_vec[first_sketch_data_idx + sketch_idx];
            }

            // sketch per thread logic
//          for (size_t thread_idx = 0; thread_idx < block_size_x; thread_idx++) {
//            size_t sketch_set = sketch_size * thread_idx;
//            size_t block_set = block_size_x * sketch_size * block_idx;
//            counts[counter_idx] = host_out_vec[block_set + sketch_set];
//            for (size_t sketch_idx = 0; sketch_idx < sketch_size - 1; sketch_idx++)
//              updated_sketch[sketch_idx] += host_out_vec[block_set + sketch_set + sketch_idx + 1];
//            counter_idx += 1;
//          }
        }
        delete host_out_vec;
      ...
      std::cout << "time taken for gpu sketch reduction: " << elapsed << std::endl;

      out_vec.push_back(counts);
      out_vec.push_back(updated_sketch);
    } else {
      for (size_t i = 0; i < agg_col_count; ++i) {
...
std::vector<int64_t*> QueryExecutionContext::launchCpuCode(const RelAlgExecutionUnit& ra_exe_unit,
                                                           ...
                                                           const uint32_t frag_stride,
                                                           const int32_t scan_limit,
                                                           std::vector<int64_t>& init_agg_vals,
                                                           int32_t* error_code,
                                                           const uint32_t num_tables,
                                                           const std::vector<int64_t>& join_hash_tables) {
  ...
  if (hoist_literals) {
    ...
    if (is_group_by) {
    ...
    } else if (is_fpd_sketch) {
      OOM_TRACE_PUSH();

      typedef void (*agg_query_sketch)(const int8_t*** col_buffers,
                                       const uint32_t* num_fragments,
                                       const uint32_t* frag_stride,
                                       const int8_t* literals,
                                       const int64_t* num_rows,
      ...
      reinterpret_cast<agg_query_sketch>(fn_ptrs[0].first)(multifrag_cols_ptr,
                                                           &num_fragments,
                                                           &frag_stride,
                                                           &literal_buff[0],
                                                           num_rows_ptr,
...
int32_t get_agg_count(const std::vector<Analyzer::Expr*>& target_exprs) {
  ...
  for (auto target_expr : target_exprs) {
    ...
    if (agg_expr && (agg_expr->get_aggtype() == kAVG || agg_expr->get_aggtype() == kFAGMS_SKETCH)) {
      agg_count += 2;
    } else {
      ++agg_count;
    }
...

  {
    ...
    if (is_group_by) {
      ...
      if (query_mem_desc.usesGetGroupValueFast() ||
        ...
        can_return_error = codegenAggCalls(agg_out_ptr_w_idx, agg_out_vec, co);
      } else {
        {
          ...
          DiamondCodegen nullcheck_cfg(LL_BUILDER.CreateICmpNE(std::get<0>(agg_out_ptr_w_idx),
                                       ...
                                       false);
          std::vector<llvm::Value*> agg_out_vec;
          codegenAggCalls(agg_out_ptr_w_idx, agg_out_vec, co);
        }
...
std::vector<std::string> agg_fn_base_names(const TargetInfo& target_info) {
  ...
  switch (target_info.agg_kind) {
    ...
    case kFAGMS_SKETCH:
      return {"agg_count_and_update_sketch"};
    default:
      abort();
  }
}
...
bool GroupByAndAggregate::codegenAggCalls(const std::tuple<llvm::Value*, llvm::Value*>& agg_out_ptr_w_idx,
  ...
  for (size_t target_idx = 0; target_idx < ra_exe_unit_.target_exprs.size(); ++target_idx) {
    ...
    if (arg_expr && agg_info.agg_kind != kFAGMS_SKETCH) {
    ...
    // TO-DO //TODO: elaboration is needed
    ...
    if (target_lvs.size() == 3 && agg_info.agg_kind != kFAGMS_SKETCH) {
    ...
    for (const auto& agg_base_name : agg_fn_names) {
      ...
      if (agg_info.agg_kind == kFAGMS_SKETCH) {
        ...
        auto fagms_sketch = executor_->getSketch().second;
        auto num_buckets = fagms_sketch->get_buckets_no();
        auto num_rows = fagms_sketch->get_rows_no();
        auto cols_to_update = executor_->getSketch().first;
        auto num_cols_to_update = cols_to_update.size();

        // number of columns to update
        auto sketch_num_cols_to_update = LL_INT(static_cast<uint8_t>(num_cols_to_update));
        agg_out_vec.push_back(sketch_num_cols_to_update);

        // PK-FK values
        auto sketch_keys = LL_BUILDER.CreateAlloca(llvm::Type::getInt64Ty(LL_CONTEXT), sketch_num_cols_to_update);
        for (unsigned i = 0; i < num_cols_to_update; i++) {
          LL_BUILDER.CreateStore(executor_->castToTypeIn(target_lvs[i], (agg_chosen_bytes << 3)),
                                 LL_BUILDER.CreateGEP(sketch_keys, LL_INT(i)));
        }
        agg_out_vec.push_back(sketch_keys);

        // indices
        auto sketch_indices = LL_BUILDER.CreateAlloca(llvm::Type::getInt32Ty(LL_CONTEXT), sketch_num_cols_to_update);
        for (unsigned i = 0; i < num_cols_to_update; i++) {
          LL_BUILDER.CreateStore(LL_INT(uint32_t(cols_to_update[i])), LL_BUILDER.CreateGEP(sketch_indices, LL_INT(i)));
        }
        agg_out_vec.push_back(sketch_indices);

        // size for each seed
        auto num_xi = LL_INT(static_cast<uint16_t>(2 * num_rows * num_cols_to_update));

        // xi_bucket
        auto sketch_xi_b = LL_BUILDER.CreateAlloca(llvm::Type::getInt32Ty(LL_CONTEXT), num_xi);
        auto xi_b = fagms_sketch->get_seed1_vals();
        for (unsigned i = 0; i < num_rows; i++) {
          for (unsigned j = 0; j < num_cols_to_update; j++) {
            LL_BUILDER.CreateStore(LL_INT(uint32_t(xi_b[j][i].seeds[0])),
                                   LL_BUILDER.CreateGEP(sketch_xi_b, LL_INT(2 * (num_cols_to_update * i + j))));
            LL_BUILDER.CreateStore(LL_INT(uint32_t(xi_b[j][i].seeds[1])),
                                   LL_BUILDER.CreateGEP(sketch_xi_b, LL_INT(2 * (num_cols_to_update * i + j) + 1)));
          }
        }
        agg_out_vec.push_back(sketch_xi_b);

        // xi_pm1
        auto sketch_xi_pm1 = LL_BUILDER.CreateAlloca(llvm::Type::getInt32Ty(LL_CONTEXT), num_xi);
        auto xi_pm1 = fagms_sketch->get_seed2_vals();
        for (unsigned i = 0; i < num_rows; i++) {
          for (unsigned j = 0; j < num_cols_to_update; j++) {
            LL_BUILDER.CreateStore(LL_INT(uint32_t(xi_pm1[j][i].seeds[0])),
                                   LL_BUILDER.CreateGEP(sketch_xi_pm1, LL_INT(2 * (num_cols_to_update * i + j))));
            LL_BUILDER.CreateStore(LL_INT(uint32_t(xi_pm1[j][i].seeds[1])),
                                   LL_BUILDER.CreateGEP(sketch_xi_pm1, LL_INT(2 * (num_cols_to_update * i + j) + 1)));
          }
        }
        agg_out_vec.push_back(sketch_xi_pm1);

        // basic info
        agg_out_vec.push_back(LL_INT(uint16_t(num_buckets)));
        agg_out_vec.push_back(LL_INT(uint16_t(num_rows)));

        if (co.device_type_ == ExecutorDeviceType::GPU) {
          const auto base_dev_addr = getAdditionalLiteral(-1);
          const auto base_host_addr = getAdditionalLiteral(-2);

          agg_out_vec.push_back(base_dev_addr);
          agg_out_vec.push_back(base_host_addr);
        }
      }
      ...
      if (is_distinct_target(agg_info)) {
        ...
        if (!agg_info.is_distinct) {
          ...
          if (co.device_type_ == ExecutorDeviceType::GPU) {
            if (agg_info.agg_kind == kFAGMS_SKETCH) {
              agg_fname += "_gpu";
              emitCall(agg_fname, agg_out_vec);
            } else {
              emitCall(agg_fname, agg_args);
            }
          } else {
            if (agg_info.agg_kind == kFAGMS_SKETCH) {
              old_val = emitCall(agg_fname, agg_out_vec);
            } else {
              old_val = emitCall(agg_fname, agg_args);
            }
          }
...
std::vector<llvm::Value*> GroupByAndAggregate::codegenAggArg(const Analyzer::Expr* target_expr,
  ...
  return agg_expr ? agg_expr->get_aggtype() != kFAGMS_SKETCH ? executor_->codegen(agg_expr->get_arg(), true, co)
                                                             : executor_->codegen(agg_expr->get_own_args(), true, co)
