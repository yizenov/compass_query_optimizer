...
class Executor {
 ...
 public:
  ...
  std::pair<std::vector<unsigned>, FAGMS_Sketch*> getSketch() { return fagms_sketch_exec; }
  std::pair<unsigned, unsigned> getTableInfo() { return table_info_exec; }
  void setSketch(std::pair<std::vector<unsigned>, FAGMS_Sketch*> sketch_data) { fagms_sketch_exec = sketch_data; }
  void setTableInfo(std::pair<unsigned, unsigned> table_data) { table_info_exec = table_data; }
  size_t getNumColsToUpdate() const { return fagms_sketch_exec.first.size(); }
  std::vector<int64_t>* getSketchInitValues() { return &init_sketch_zero_values; }
  void setSketchInitValues(std::vector<int64_t>& sketch_values) { init_sketch_zero_values = sketch_values; }
 ...
 private:
  ...
  std::vector<llvm::Value*> codegen(const std::vector<std::shared_ptr<Analyzer::Expr>> exprs, const bool fetch_columns, const CompilationOptions& co);
  ...
  std::pair<std::vector<unsigned>, FAGMS_Sketch*> fagms_sketch_exec;
  std::pair<unsigned, unsigned> table_info_exec;
  std::vector<int64_t> init_sketch_zero_values;
