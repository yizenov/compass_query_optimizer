...
std::vector<llvm::Value*> Executor::codegen(const std::vector<std::shared_ptr<Analyzer::Expr>> exprs,
                                            const bool fetch_columns,
                                            const CompilationOptions& co) {
  std::vector<llvm::Value*> fetched_exprs;
  for (auto expr : exprs) {
    auto col_var = dynamic_cast<const Analyzer::ColumnVar*>(expr.get());
    CHECK(col_var);
    auto fetched_expr = codegen(col_var, fetch_columns, co);
    CHECK_EQ(size_t(1), fetched_expr.size());
    fetched_exprs.push_back(fetched_expr[0]);
  }
  return fetched_exprs;
}
